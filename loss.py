...
class IoU2(nn.Module):
    def __init__(self):
        super(IoU2, self).__init__()

    def iou(self, y_true, y_pred):
        smooth = 1e-7  # may change
        ...
        score = intersection / (i + j - intersection + smooth)  # iou
        return score.mean()

    def __call__(self, y_true, y_pred):
        b = self.iou(y_true, y_pred)
        return b

    def metric_iou(self, y_true, y_pred):
        thresholds = range(1, 8)
        iou_threshold = torch.empty(len(thresholds))
        for i, threshold in enumerate(thresholds):
            new_y_pred = copy.deepcopy(y_pred)
            new_y_pred[y_pred < threshold] = 0
            new_y_pred[y_pred >= threshold] = 1
            iou_threshold[i] = self.iou(y_true, new_y_pred)
